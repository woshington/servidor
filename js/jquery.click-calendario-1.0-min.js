...
jQuery.fn.calendario=function(options){
	var settings={
		target:'',targetDay:'',targetMonth:'',targetYear:'',minDate:'',maxDate:'',dateDefault:dataAtual,left:'0',top:'30',referencePosition:this,closeClick:true
	};
	...
	this.each(
		function(){
			...
			function preencher_calendario(idCalendario){
				...
				$('#'+idCalendario+' ul.lista_dia li a').click(
					function(){
			...
			$('#'+idCalendario).css({'left':leftPosition,'top':topPosition});
			$('#'+idCalendario).show();
		}
	);
};